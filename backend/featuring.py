...
class SubPackageFeaturing():
    ...
    def saveNew10PopularIntruments(self, new10PopularInstruments: List[str]) -> None:
        ...
        if any([instrument not in list(map(lambda i: i.instrument, allIntruments)) for instrument in new10PopularInstruments]):
            raise Exception('Invalid new popular intrument.')

        [databaseSubsystem.upsert(instrument.instrument_class, instrument.instrument, instrument.brand, instrument.model, instrument.registry in new10PopularInstruments) for instrument in allIntruments]
